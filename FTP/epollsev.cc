...
void epoll_write(int fd){
    ...
    std::lock_guard<std::mutex> lock(clients[fd].mutex);
    {
        while(!clients[fd].writebuf.empty()){
            n = send(fd, clients[fd].writebuf.data(),
                     clients[fd].writebuf.size(), 0);
            if(n>0){
...
void do_pasv(int connfd) {
    ...
    clients[connfd].datafd = accept(datalfd, NULL, NULL);
    close(datalfd);
}
...
void do_list(int connfd){
    if ( clients[connfd].datafd < 0) {
        std::string answer = "425 No data connection\r\n";
        std::lock_guard<std::mutex> lock(clients[connfd].mutex);
        {
            clients[connfd].writebuf += answer;
            epoll_mod(connfd, EPOLLIN | EPOLLOUT | EPOLLET);
        }
        return;
    }
...
void do_retr(int connfd, const std::vector<std::string> commonds) {
    ...
    while (offset < st.st_size) {
        ssize_t sent =
            sendfile( clients[connfd].datafd, file_fp, &offset, st.st_size - offset);
        if (sent <= 0)
            break;
    }
    fclose(fp);
...
void do_stor(int connfd, const std::vector<std::string> commonds) {
    ...
    while (1) {
        int len = recv( clients[connfd].datafd, file, sizeof(file), 0);
        if (len <= 0) {
            break;
        }
        fp.write(file, len);
...
void epoll_read(int connfd) {
    ...
    while (1) {
        ...
        if (commonds.empty())
            continue;
        if (commonds[0] == "USER") {
            clients[connfd].nowuser = commonds.size() > 1 ? true : false;
            std::lock_guard<std::mutex> lock(clients[connfd].mutex);
            {
                clients[connfd].writebuf += "331 User name ok, need password\r\n";
                epoll_mod(connfd, EPOLLIN | EPOLLOUT | EPOLLET);
            }
            return;
...
int main() {
    ...
    events[listenfd].events = EPOLLIN;
    events[listenfd].data.fd = listenfd;
    epoll_ctl(epfd, EPOLL_CTL_ADD, listenfd, &events[listenfd]);
